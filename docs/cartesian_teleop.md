...
- 左 Grip：按下时锚定当前姿态为零位；左手姿态控制 Base 系 XYZ 平移速度。
- 右 Grip：按下时锚定当前姿态为零位；右手姿态控制末端执行器 XYZ 旋转速度，每拍再统一转换为 Base 系增量执行。
- 左手 Roll/Pitch/Yaw 分别映射到 Base X/Y/Z；右手姿态 X/Y 互换映射、Z 保持不变。默认 `10°` 死区，超过死区后从 0 线性增加，`25°` 达到每帧动作上限；保持倾斜会持续运动，回到零位或松开 Grip 会停止。
